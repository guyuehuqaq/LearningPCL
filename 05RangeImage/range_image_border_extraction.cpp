...
#include <pcl/features/range_image_border_extractor.h>  // 深度图提取边界的库
#include <pcl/visualization/range_image_visualizer.h>
#include <pcl/visualization/pcl_visualizer.h>
#include <pcl/console/parse.h>
#include <pcl/common/file_io.h> // 扩展名处理
...
int main(int argc, char **argv)
{
  ...
  pcl::PointCloud<pcl::PointWithViewpoint> far_ranges;  // PointWithViewpoint: 增加了视点，即看到这个点的相机位置
  Eigen::Affine3f scene_sensor_pose (Eigen::Affine3f::Identity ());  // 传感器的位置
  // 找到命令行中有"pcd"的参数的索引
  std::vector<int> pcd_filename_indices = pcl::console::parse_file_extension_argument (argc, argv, "pcd");
  // 找pcd文件的数据，点云文件会记录一帧点云数据，包括记录的相机的姿态
  ...
  else
  {
    ...
    for (float x=-0.5f; x<=0.5f; x+=0.01f)
    {
      ...
      {
        PointType point;  point.x = x;  point.y = y;  point.z = 1.0f; // point.z = 2.0f - y;
...
//  viewer.addPointCloud (point_cloud_ptr, point_cloud_color_handler, "original point cloud");   //添加点云
  ...
  pcl::PointCloud<pcl::BorderDescription> border_descriptions;  // 定义输出点云，存放每个像素的边界描述
  border_extractor.compute(border_descriptions);     // 结果填充到border_descriptions中，每个点描述对应像素的边界信息
