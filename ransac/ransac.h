  ...
  void Check() const {
    if (max_error <= 0) {
      AlgLogger::GetAlgLogger()->critical("[file:{} line:{} fun:{}]:{}",
                                          ...
                                          __LINE__,
                                          __FUNCTION__,
                                          "max_error must be greater than 0.");
      exit(1);
    }
    if (min_inlier_ratio < 0) {
      AlgLogger::GetAlgLogger()->critical(
          ...
          __LINE__,
          __FUNCTION__,
          "min_inlier_ratio must be greater than 0.");
      exit(1);
    }
    if (min_inlier_ratio > 1) {
      AlgLogger::GetAlgLogger()->critical(
          "[file:{} line:{} fun:{}]:{}",
          FILENAME,
          __LINE__,
          __FUNCTION__,
          "min_inlier_ratio must be less than 1.");
      exit(1);
    }
    if (confidence < 0) {
      AlgLogger::GetAlgLogger()->critical("[file:{} line:{} fun:{}]:{}",
    ...
    if (min_num_trials > max_num_trials) {
      AlgLogger::GetAlgLogger()->critical(
          ...
          __LINE__,
          __FUNCTION__,
          "min_num_trials must be less than max_num_trials.");
      exit(1);
    }
  }
};
